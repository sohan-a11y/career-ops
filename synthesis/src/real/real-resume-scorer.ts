...
import type { ScoreResult, FinalDecision, RiskLevel, ConfidenceLevel } from '../types/score-result.js';
...
const VALID_FINAL_DECISIONS: readonly FinalDecision[] = ['Apply', 'Consider', 'Research first', 'Skip'];
...
interface ScoreResponse {
  score?: number;
  dimensions?: { matchWithCv?: number; northStarAlignment?: number; comp?: number; culturalSignals?: number };
...
function coerceEnum<T extends string>(value: string | undefined, valid: readonly T[], fallback: T): T {
 ...
 */
function readCultureScreen(careerOpsRoot: string): unknown {
  const profilePath = join(careerOpsRoot, 'config', 'profile.yml');
  if (!existsSync(profilePath)) return null;
  try {
    const parsed = yaml.load(readFileSync(profilePath, 'utf-8')) as { culture_screen?: unknown } | undefined;
...
export class RealResumeScorer implements IResumeScorer {
  constructor(private readonly opts: RealResumeScorerOptions) {}

  async score(portfolio: MergedPortfolio, matrix: TargetMatrix): Promise<ScoreResult> {
    ...
    const raw = invokeHeadlessAi('score', systemPrompt, userPrompt, this.opts) as ScoreResponse;
