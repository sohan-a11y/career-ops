...
import type { MutablePayload, MutableEmploymentData } from '../types/mutable-payload.js';
...
// ── Pipeline event types (for telemetry / dashboard) ───────────────

export type PipelineEvent =
  | { readonly kind: 'stage-start';    readonly stage: SynthesisStage; readonly timestamp: string }
  | { readonly kind: 'stage-complete'; readonly stage: SynthesisStage; readonly durationMs: number; readonly timestamp: string }
  | { readonly kind: 'stage-error';    readonly stage: SynthesisStage; readonly error: string; readonly timestamp: string }
  | { readonly kind: 'merge-stats';    readonly stats: MergeStats; readonly timestamp: string }
  | { readonly kind: 'score-result';   readonly result: ScoreResult; readonly timestamp: string }
  | { readonly kind: 'pipeline-done';  readonly totalMs: number; readonly timestamp: string };
...
export class DynamicPortfolioOrchestrator {
  private readonly analyzer: ITargetAnalyzer;
  private readonly synthesizer: IPortfolioSynthesizer;
  private readonly tailor: IExperienceTailor;
  private readonly extractor: IDataExtractor;
  private readonly compiler: IDocumentCompiler;
  private readonly scorer: IResumeScorer | undefined;
  ...
  async execute(
    ...
    // ── Stage 1: Extract ────────────────────────────────────────
    const extracted = await this.runStage('extract', () =>
      this.extractor.extract(source),
    );

    // ── Stage 2: Analyze ────────────────────────────────────────
    const matrix = await this.runStage('analyze', () =>
      this.analyzer.analyze(extracted.text),
    );
    ...
    // ── Stage 5: Merge ──────────────────────────────────────────
    const merged = await this.runStage('merge', async () =>
      this.mergeState(profile, payload, { targetCompany: matrix.companyName, targetRole: matrix.roleTitle, targetLocation: matrix.location }),
    );

    // ── Stage 6: Compile ────────────────────────────────────────
    const compiled = await this.runStage('compile', () =>
      this.compiler.compile(merged),
    );
    ...
    if (this.scorer) {
      const scoreResult = await this.runStage('score', () =>
        this.scorer!.score(merged, matrix),
      );
      this.emit({
        kind: 'score-result',
        result: scoreResult,
  ...
  async mergeOnly(
    ...
    target?: { targetCompany?: string; targetRole?: string; targetLocation?: string },
  ...
  private mergeState(
    ...
    const immutableIds = new Set(
      profile.employmentHistory.map(e => e.companyId),
    );

    // Detect orphaned mutable keys (payload entries with no profile match).
    const orphanedMutableKeys: string[] = [];
    ...
    if (this.config.strictMerge && orphanedMutableKeys.length > 0) {
      throw new MergeConflictError(
        `Strict merge failed: ${orphanedMutableKeys.length} mutable key(s) have no matching companyId in the immutable profile: [${orphanedMutableKeys.join(', ')}]`,
        orphanedMutableKeys,
      );
    }
    ...
    const employment: MergedEmploymentEntry[] =
      profile.employmentHistory.map(
        (record: EmploymentRecord): MergedEmploymentEntry => {
          const mutable: MutableEmploymentData | undefined =
            payload.employmentDetails.get(record.companyId);
    ...
    const stats: MergeStats = Object.freeze({
   ...
   */
  private async runParallelStages<A, B>(
    a: { stage: SynthesisStage; fn: () => Promise<A> },
    b: { stage: SynthesisStage; fn: () => Promise<B> },
  ): Promise<[A, B]> {
  ...
  private timeoutPromise(stage: SynthesisStage): Promise<never> {
  ...
  private checkAborted(stage: SynthesisStage): void {
