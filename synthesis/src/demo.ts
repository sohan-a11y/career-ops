...
import type { PipelineEvent } from './orchestrator/dynamic-portfolio-orchestrator.js';
...
const SAMPLE_PROFILE: ImmutableProfile = {
  contactInfo: {
    name: 'Alex Chen',
...
function logEvent(event: PipelineEvent): void {
...
async function main(): Promise<void> {
  ...
  const portfolio: MergedPortfolio = JSON.parse(compiled.buffer.toString('utf-8'));
  ...
  for (const project of portfolio.tailoredProjects as Array<{ name: string; description: string; techStack: string[]; relevanceScore: number }>) {
  ...
  for (const edu of portfolio.education as Array<{ institution: string; degree: string; year: number }>) {
