...
function makeDeps(overrides = {}) {
    return {
        extractor: overrides.extractor ?? {
            extract: async (source) => ({ text: source, title: '', resolvedUrl: source, metadata: {}, extractedAt: new Date().toISOString() }),
        },
        ...
        tailor: overrides.tailor ?? {
            tailor: async (_matrix, companyIds) => {
                const m = new Map();
                for (const id of companyIds)
                    m.set(id, { tailoredTitle: `Tailored ${id}`, highlights: [`did ${id}`] });
                return m;
            },
        },
...
async function testTargetMetadataPropagation() {
    ...
    const portfolio = capturedPortfolio;
...
async function testStageTimeout() {
    const orchestrator = new DynamicPortfolioOrchestrator(makeDeps({ analyzer: { analyze: async () => { await new Promise((r) => setTimeout(r, 200)); return { roleTitle: '', companyName: '', requiredSkills: [], preferredSkills: [], responsibilityThemes: [], industryContext: [], senioritySignal: '', location: '', rawSource: '' }; } } }), { stageTimeoutMs: 20 });
    let threw = false;
    try {
        await orchestrator.execute('jd', PROFILE);
    }
...
async function testScoreStageRunsWhenScorerProvided() {
    ...
    check(capturedPortfolio?.targetCompany === 'Target Co', 'the scorer receives the SAME merged portfolio that was compiled, not a separate copy');
    check(capturedMatrix?.companyName === 'Target Co', 'the scorer receives the target matrix from Analyze');
    ...
    const compileCompleteIndex = events.findIndex((e) => e.kind === 'stage-complete' && e.stage === 'compile');
...
main();
//# sourceMappingURL=dynamic-portfolio-orchestrator.test.js.map