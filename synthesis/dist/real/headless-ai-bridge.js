 ...
 */
export function resolveModel(careerOpsRoot, explicitModel) {
    if (explicitModel)
    ...
    return TIER_TO_MODEL[tier] ?? TIER_TO_MODEL.standard;
}
/**
 ...
 */
export function buildSystemPrompt(careerOpsRoot, stageInstructions) {
    const parts = [
 ...
 * response block.
 */
export function invokeHeadlessAi(stage, systemPrompt, userPrompt, opts) {
    ...
    try {
        ...
        const result = spawnSync(claudeBinary, ['-p', '--dangerously-skip-permissions', '--strict-mcp-config', '--model', model, '--append-system-prompt-file', systemPromptFile, userPrompt], { encoding: 'utf-8', timeout: timeoutMs, maxBuffer: 50 * 1024 * 1024 });
        if (result.error) {
...
//# sourceMappingURL=headless-ai-bridge.js.map