...
function splitCertification(raw) {
    const match = raw.match(/^(.+?)\s*\((\d{4})\)\s*$/);
    if (match)
        return { title: match[1] ?? raw, org: '', year: match[2] ?? '' };
    return { title: raw, org: '', year: '' };
}
function toEducationEntry(raw) {
    const e = raw;
    return { title: e?.degree ?? '', org: e?.institution ?? '', year: e?.years ?? '' };
}
function toProjectEntry(raw) {
    const p = raw;
 ...
 * already follows.
 */
export function detectPageFormat(location) {
    if (!location || !location.trim())
        return 'letter';
    const text = location.toLowerCase();
    if (US_CANADA_COUNTRY_PATTERNS.some((p) => p.test(text)))
        return 'letter';
    ...
    if (segments.some((seg) => US_STATE_CODES.has(seg)))
        return 'letter';
    return 'a4';
}
function buildRenderPayload(portfolio, lang, pageFormat) {
    return {
        lang,
...
function readOutputLanguage(careerOpsRoot) {
...
export class RealDocumentCompiler {
    opts;
    constructor(opts) {
        this.opts = opts;
    }
    async compile(portfolio) {
        ...
        if (this.opts.reportNumber)
            pdfArgs.push(`--report=${this.opts.reportNumber}`);
    ...
    runScript(stage, script, args, timeoutMs, opts = {}) {
...
//# sourceMappingURL=real-document-compiler.js.map