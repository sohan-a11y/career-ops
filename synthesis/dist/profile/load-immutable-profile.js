 ...
 */
export function parseExperienceEntries(experienceSection) {
 ...
 */
export function splitDateRange(raw) {
    ...
    let parts = [withoutParenthetical];
 ...
 */
export function assignCompanyIds(entries) {
 ...
 * collide).
 */
export function buildEmploymentHistory(entries) {
 ...
 */
export function extractRawBulletsByCompanyId(entries) {
    const map = new Map();
 ...
 * display strings.
 */
export function parseEducationAndCertifications(markdown) {
    ...
    for (const rawLine of lines) {
        ...
        if (inCertBlock) {
            const bulletMatch = line.match(/^[-*]\s+(.+)$/);
            if (bulletMatch?.[1])
                certifications.push(bulletMatch[1].trim());
            continue;
        }
 ...
 * placeholder.
 */
export function buildContactInfo(candidate) {
    if (!candidate)
        return {};
    const fields = [
 ...
 */
export function loadImmutableProfile(opts) {
    ...
    let profileYaml;
    try {
        profileYaml = yaml.load(profileRaw) ?? {};
    }
    catch (err) {
...
//# sourceMappingURL=load-immutable-profile.js.map