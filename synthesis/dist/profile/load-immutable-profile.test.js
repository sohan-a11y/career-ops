...
import { extractSection, parseExperienceEntries, splitDateRange, buildEmploymentHistory, assignCompanyIds, extractRawBulletsByCompanyId, parseEducationAndCertifications, buildContactInfo, loadImmutableProfile, ProfileLoadError, } from './load-immutable-profile.js';
let pass = 0;
let fail = 0;
...
// ── assignCompanyIds / extractRawBulletsByCompanyId ─────────────────
{
    ...
    check(history.every((h) => bulletsByCompanyId.has(h.companyId)), 'every companyId in employmentHistory has a matching entry in extractRawBulletsByCompanyId — the merge join can never silently drop one');
    check(JSON.stringify(bulletsByCompanyId.get('acme-corp')) === JSON.stringify(['did X', 'did Y']), 'extractRawBulletsByCompanyId returns the exact source bullets, unmodified');
}
// ── splitDateRange ───────────────────────────────────────────────────
{
...
// ── parseEducationAndCertifications ─────────────────────────────────
{
    ...
    check(education[0].degree === 'B.Tech, Computer Science', 'education degree parsed');
...
//# sourceMappingURL=load-immutable-profile.test.js.map